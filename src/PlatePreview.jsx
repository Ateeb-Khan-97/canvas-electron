...
export default forwardRef(function PlatePreview(props, ref) {
  ...
  useEffect(() => {
    ...
    const plateTemp = container.firstElementChild;

    // const height = plate.plateSize.height;
    // const width = plate.plateSize.width;

    const scale = Math.min(
      container.offsetWidth / plateTemp.scrollWidth,
      container.offsetHeight / plateTemp.scrollHeight
    );

    // plateTemp.classList.add("rotate-90");

    // plate.style.transform = `translate(-50%, -50%) scale(${scale})`;
    plateTemp.style.setProperty("--tw-scale-x", scale);
    plateTemp.style.setProperty("--tw-scale-y", scale);
  }, [plate.plateSize]);
  ...
  return (
    <article
      id={plate.type === "front" ? "container-front" : "container-rear"}
      className="bg-checkered bg-[length:20px_20px] bg-center overflow-hidden w-full min-h-96 flex items-center justify-center mt-4 p-2 relative print:bg-none preview-article"
      ref={ref}
    >
      <div
        ...
        style={{
          ...
          paddingLeft: plate.isBadgeEnabled
            ? `calc(45mm + ${plate.borderMargin}mm)`
            : `${plate.borderMargin}mm`,
      ...
      >
        <div
          ...
          style={{
            outline: plate.isBorderEnabled
              ? `black solid ${plate.borderWidth}mm`
              : "none",
        ...
        >
          <div
            ref={textRef}
            className="flex justify-center items-center whitespace-nowrap w-full h-full relative"
            style={{
              // fontSize: "79mm",
              lineHeight: 1,
            }}
          >
            <h1 className="text-[800%] custom-font">{ctx.letters}</h1>
          </div>
          ...
          {plate.bsau !== "" && !plate.isBsauPrePrinted && (
            <div
              ...
              style={{
                ...
                fontStyle: `${ctx.postCodeFontStyle}mm`,
                lineHeight: 1,
                paddingTop: plate.isBsauOnBorder
                  ? `calc(${plate.borderWidth}mm - 0.9lh)`
                  : "0",
                bottom: plate.isBsauOnBorder
                  ? "-2px"
                  : `calc(${plate.borderWidth}mm + 2mm)`,
